...
class AortaSegmentation(InferenceClass):
    ...
    def __call__(self, inference_pipeline):
        ...
        inference_pipeline.output_dir_segmentations = os.path.join(self.output_dir, "segmentations/")
...
class AorticCalciumSegmentation(InferenceClass):
    ...
    def __call__(self, inference_pipeline):   
        ...
        inference_pipeline.calc_mask = self.detectCalcifications(ct, aorta_mask, exclude_mask=spine_mask)
    ...
    def detectCalcifications(self, ct, aorta_mask, exclude_mask=None, return_dilated_mask = False, dilation=(3,1), 
                             dilation_iteration = 4, return_dilated_exclude=False, dilation_exclude_mask=(3,1), 
                             dilation_iteration_exclude=3, show_time=False, num_std = 3, remove_size = None,
                             verbose = False, exclude_center_aorta=True, return_eroded_aorta=False, aorta_erode_iteration=6):
        '''
        ...
        def slicedDilationOrErosion(input_mask, struct, num_iteration, operation):
            ...
            if operation == 'dilate':
                mask_slice = ndimage.binary_dilation(input_mask[x_start:x_end, y_start:y_end,:], structure=struct).astype(np.int8)
            elif operation == 'erode':
                mask_slice = ndimage.binary_erosion(input_mask[x_start:x_end, y_start:y_end,:], structure=struct).astype(np.int8)
        ...
        if num_classes > 1:
            if verbose: print('Removing {} parts'.format(num_classes-1))
        ...
        if dilation is not None:
            ...
            aorta_dilated = slicedDilationOrErosion(aorta_mask, struct=struct, num_iteration=dilation_iteration, operation='dilate')
            
            if show_time: print('dilation mask time: {:.2f}'.format(time.time()-t0))
        
        t0 = time.time()
        calc_mask = np.logical_and(aorta_dilated == 1, ct >= calc_thres)
        if show_time: print('find calc time: {:.2f}'.format(time.time()-t0))
        ...
        if exclude_center_aorta:
            ...
            aorta_eroded = slicedDilationOrErosion(aorta_mask, struct=struct, num_iteration=aorta_erode_iteration, operation='erode')
            calc_mask = calc_mask * (aorta_eroded == 0)
            if show_time: print('exclude center aorta time: {:.2f} sec'.format(time.time()-t0))
        ...
        if exclude_mask is not None:
            if dilation_exclude_mask is not None:
                ...
                if dilation_iteration_exclude is not None:
                    struct_exclude = ndimage.iterate_structure(struct_exclude, dilation_iteration_exclude)
                
                exclude_mask = slicedDilationOrErosion(exclude_mask, struct=struct_exclude, num_iteration=dilation_iteration_exclude, operation='dilate')
                
            if show_time: print('exclude dilation time: {:.2f}'.format(time.time()-t0))
            
            t0 = time.time()
            calc_mask = calc_mask * (exclude_mask == 0)
            if show_time: print('exclude time: {:.2f}'.format(time.time()-t0))
        ...
        if remove_size is not None:
            ...
            if show_time: print('Size exclusion time: {:.1f} sec'.format(time.time()-t0))
            if verbose: print('Excluded {} foci under {}'.format(counter, remove_size))    
...
class AorticCalciumMetrics(InferenceClass):
    ...
    def __call__(self, inference_pipeline):   
        ...
        calc_vol = calc_mask.sum() * inference_pipeline.vol_per_pixel
