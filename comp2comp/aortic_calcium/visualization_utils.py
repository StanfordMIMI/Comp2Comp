...
def createMipPlot(
    ...
    plane_mask_proj = np.where(
        np.flip(np.transpose((plane_mask == 1).max(axis=(0, 1))), axis=0)
    )[0][0]
    ...
    axx.plot(
        [0, ct_proj_all.shape[1]],
        [plane_mask_proj, plane_mask_proj],
        color=map_object_seg(3),
        lw=0.8,
        alpha=0.8,
        zorder=0,
    )
    ...
    custom_bbox = Bbox(
        [
            [
                tight_bbox.x0 - 0.05,
                tight_bbox.y0 - 0.07,
            ],  # Add 0.5 inches to the left only
    ...
    for i, (region, region_metrics) in enumerate(metrics.items()):
        ...
        report_text.append(
            "{:<{}}{:<{}}{}".format(
                "", indent, "Total number:", spacing, region_metrics["num_calc"]
            )
        )
        report_text.append(
            "{:<{}}{:<{}}{:.3f}".format(
        ...
        report_text.append(
            "{:<{}}{:<{}}{:.3f}{}{:.3f}".format(
                ...
                "Mean volume (cm³):",
                spacing,
                np.mean(region_metrics["volume"]),
                r"$\pm$",
                np.std(region_metrics["volume"]),
            )
        )
        report_text.append(
            "{:<{}}{:<{}}{:.1f}{}{:.1f}".format(
                ...
                "Median HU:",
                spacing,
                np.mean(region_metrics["median_hu"]),
                r"$\pm$",
                np.std(region_metrics["median_hu"]),
            )
        )
        report_text.append(
            "{:<{}}{:<{}}{:.3f}".format(
        ...
        if "agatston_score" in region_metrics:
            report_text.append(
                "{:<{}}{:<{}}{:.0f}".format(
                    "", indent, "Agatston:", spacing, region_metrics["agatston_score"]
                )
            )
    ...
    report_text.append(
        "{:<{}}{:<{}}{}".format("", indent, "Threshold (HU):", spacing, HU_val)
    )
    ...
    custom_bbox_text = Bbox(
        [
            [
                tight_bbox_text.x0 - 0.05,
                tight_bbox_text.y0 - 0.14,
            ],  # Add 0.5 inches to the left only
    ...
    buf_text = io.BytesIO()
    fig_t.savefig(
        buf_text, bbox_inches=custom_bbox_text, pad_inches=0, dpi=300, format="png"
    )
    plt.close(fig_t)
    buf_text.seek(0)
    ...
    image_text_resample = image_text.resize(
        [adjusted_width, image_mip.height], Image.LANCZOS
    )

    # Merge into one image
    result = Image.new(
        "RGB", (image_mip.width + image_text_resample.width, image_mip.height)
    )
...
def createCalciumMosaicAll(
    ...
    if len(ct_tmp) > 0:
        ...
        aorta_dil_crops.append(
            np.pad(
                np.hstack(aorta_dil_tmp),
                ((0, 0), (0, pad_len)),
                mode="constant",
                constant_values=0,
            )
    ...
    fig.savefig(
        os.path.join(path, "mosaic.png"), bbox_inches="tight", pad_inches=0, dpi=300
    )
...
def mergeMipAndMosaic(save_root: str) -> None:
    ...
    if os.path.isfile(os.path.join(save_root, "sub_figures/mosaic.png")):
        ...
        img_mosaic_resample = img_mosaic.resize(
            [img_proj.width, new_height], Image.LANCZOS
        )

        result = Image.new(
            "RGB", (img_proj.width, img_proj.height + img_mosaic_resample.height)
        )
        ...
        shutil.copy2(
            os.path.join(save_root, "sub_figures/projection.png"),
            os.path.join(save_root, "overview.png"),
        )
