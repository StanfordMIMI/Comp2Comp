...
class AortaSegmentation(InferenceClass):
    ...
    def __call__(self, inference_pipeline):
        ...
        # Save axial aorta masks to pipeline
        inference_pipeline.axial_masks = axial_masks

        # Save slices to pipeline
        inference_pipeline.ct_image = ct_image
...
class AortaDiameter(InferenceClass):
    ...
    def __call__(self, inference_pipeline):
        ...
        # sagittal_masks = np.rot90(inference_pipeline.axial_masks)
        # sagittal_img = np.rot90(inference_pipeline.ct_image)

        # coronal_masks = np.transpose(inference_pipeline.axial_masks)
        # coronal_img = np.transpose(inference_pipeline.ct_image)
        ...
        for i in range(len(ct_img)):
            ...
            if len(contours) != 0:
                ...
                #   if len(contours[i]) >= 5:
                    ...
                    # xtop1 = xc + math.cos(math.radians(angle+180))*rminor
                    # ytop1 = yc + math.sin(math.radians(angle+180))*rminor
                    # xbot1 = xc + math.cos(math.radians(angle+180))*rminor
                    # ybot1 = yc + math.sin(math.radians(angle+180))*rminor
                    # cv2.line(img, (int(xtop1),int(ytop1)), (int(xbot1),int(ybot1)), (0, 255, 0), 3)

                    # ## BLUE

                    # xtop1 = xc + math.cos(math.radians(angle+180))*rminor
                    # ytop1 = yc + math.sin(math.radians(angle+180))*rminor
                    # xbot1 = xc + math.cos(math.radians(angle))*rminor
                    # ybot1 = yc + math.sin(math.radians(angle))*rminor
                    # cv2.line(img, (int(xtop1),int(ytop1)), (int(xbot1),int(ybot1)), (255, 0, 0), 3)
                    ...
                    # cX, cY, dX, dY = getPerpCoord(xtop, ytop, xbot, ybot, (rminor))


                    # center = xc, yc
                    # ellipse_area = math.pi * d1 * d2
            


                    # cv2.line(img, (int(cX-(rmajor)),int(cY)), (int(dX-(rmajor)),int(dY)), (0, 0, 255), 3)
