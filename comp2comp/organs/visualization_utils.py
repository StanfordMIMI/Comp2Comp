...
def save_slice(ct_slice, mask_slice, path, figsize=(12,12), corner_text=None, unit_dict = None, aspect=1, show=False,
               xy_placement = None , class_color=1, fontsize=14):
    ...
    axx.imshow(ct_slice, cmap='gray', vmin=-400, vmax=400, interpolation='spline36', aspect=aspect, origin='lower')
    axx.imshow(mask_slice*class_color, cmap=map_object_seg, vmin=0,vmax=9, alpha=0.2, 
    ...
    if corner_text is not None:
        ...
        if xy_placement is None:
            ...
            t = axx.text(0.5, 0.5, '\n'.join(texts), color='white', transform=axx.transAxes,
                      fontsize=fontsize, family='monospace', bbox=bbox_props, va='top', ha='left')
        ...
        axx.text(xy_placement[0], xy_placement[1], '\n'.join(texts), color='white', transform=axx.transAxes,
                  fontsize=fontsize, family='monospace', bbox=bbox_props, va='top', ha='left')
...
def extract_organ_metrics(ct, all_masks, class_num=None, vol_per_pixel=None):
    ...
    # in ml
    organ_vol = len(ct_organ_vals)*vol_per_pixel
...
def generate_slice_images(ct, all_masks, class_nums, unit_dict, vol_per_pixel, pix_dims, root, 
    ...
    for i, c_num in enumerate(class_nums):
        ...
        save_slice(ct_slice_z, liver_slice_z, axial_path, figsize=(12,12), corner_text=results, unit_dict=unit_dict,
                   class_color=colors[i], fontsize=fontsize, show=show)
        
        ct_slice_y, liver_slice_y = extract_coronal_mid_slice(ct, all_masks == c_num)
        
        save_slice(ct_slice_y, liver_slice_y, coronal_path, figsize=(12,12), aspect=pix_dims[2]/pix_dims[1], 
                   show=show, class_color=colors[i])
...
def generate_liver_spleen_pancreas_report(root, organ_names):
    ...
    result_width = max(sum([img.size[0] for img in axial_imgs]),
