...
import os
import numpy as np
from comp2comp.organs.visualization_utils import generate_slice_images, generate_liver_spleen_pancreas_report
...
class LiverSpleenPancreasVisualizer(InferenceClass):
    ...
    def __call__(self, inference_pipeline):
        ...
        inference_pipeline.output_dir_images_organs_organs_organs = os.path.join(self.output_dir, "images/organs/")
        ...
        inference_pipeline.vol_per_pixel = np.prod(inference_pipeline.pix_dims/10) # mm to cm for having ml/pixel.
        
        self.organ_metrics = generate_slice_images(inference_pipeline.medical_volume_arr, 
                                                   inference_pipeline.segmentation_arr, 
                                                   self.class_nums, self.unit_dict, inference_pipeline.vol_per_pixel, inference_pipeline.pix_dims, 
                                                   self.output_dir_images_organs, fontsize=24)
...
class LiverSpleenPancreasMetricsPrinter(InferenceClass):
     ...
     def __call__(self, inference_pipeline):
        ...
        with open(os.path.join(self.output_dir_metrics_organs, 'liver_spleen_pancreas_metrics.csv'), 'w') as f:
            f.write(header)
